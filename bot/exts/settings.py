...
class Settings(commands.Cog):
    ...
    async def preview(self, ctx: commands.Context):
        ...
        board, piece = Encoder.decode(
            'ACIAAAAAAlUAATMCZVdxMAZmF3EwAEQVUXcEQhNVdwIiEzM=@6+16+-1+1'
        )
        ghost = piece.copy()
        ghost.x += 30
    ...
    async def settings(
        self, ctx: commands.Context, name: str = None, value: str = None
    ):
        ...
        if name is None:
            await ctx.send(
                embed=discord.Embed(
                    ...
                    title='Your settings',
                    description='\n'.join(
                        f'**`{k}`**: `{names[k][v]}`' for k, v in user_settings.items()
                    ),
                )
            )
            ...
            await ctx.send(
                f'`{name}` is set to `{names[name][user_settings[name]]}` of'
                f' `{"`, `".join(names[name])}`'
            )
